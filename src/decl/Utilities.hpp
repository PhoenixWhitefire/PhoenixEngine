#pragma once

#define RAISE_RT_NF(error) throw(std::runtime_error(error))
#define RAISE_RT(fmtstr, ...) RAISE_RT_NF(std::format(fmtstr "" __VA_OPT__(,) __VA_ARGS__))

// 24/01/2025
// an assert, but also at runtime
// can be useful, because important exceptions
// can end up being directed to the exception handlers
// in the `main` function, preventing VS debugger
// from being invoked directly at the point of the exception
#ifdef NDEBUG

#define PHX_ENSURE_MSG(expr, err) do { if (!(expr)) RAISE_RT_NF(err); } while (false)
#define PHX_ENSURE(expr) do { if (!(expr)) RAISE_RT_NF("Failed to ensure: " #expr); } while (false)

#define PHX_CHECK(expr) do { if (!(expr)) Log.Error( "'" #expr "' failed"); } while (false)

#else

#define PHX_ENSURE_MSG(expr, err) assert(expr)
#define PHX_ENSURE assert

#define PHX_CHECK assert

#endif

#include <format>
#include <assert.h>

#include "Log.hpp"

double GetRunningTime();
